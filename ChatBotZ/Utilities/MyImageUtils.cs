    ...
    internal class MyImageUtils
    {
        ...
        private static async Task<byte[]> ProcessSection(string apiKey, string prompt, byte[] imageBytes, int size, bool useMask = false)
        {
            try
            {
                ...
                MyOpenAIAPI openAI = new MyOpenAIAPI();
                ...
                if (useMask)
                {
                    ...
                    imageUrl = await MyOpenAIAPI.EditImageAsyncHttp(apiKey, prompt, imageBytes, maskBytes, $"{size}x{size}");
                }
                else
                {
                    imageUrl = await MyOpenAIAPI.EditImageAsyncHttp(apiKey, prompt, imageBytes, $"{size}x{size}");
                }
                processedSection = await GetImageAsByteArray(imageUrl);
        ...
        public static async Task<byte[]> ProcessImage(string apiKey, string prompt, byte[] imageBytes)
        {
            ...
            for (int x = 0; x < sectionWidth; x++)
            {
                for (int y = 0; y < sectionHeight; y++)
                {
                    ...
                    Toast.MakeText(Application.Context, $"Current Section: X:{startX}-{startX + workSectionWidth}, Y:{startY}-{startY + workSectionHeight}...", ToastLength.Long).Show();
                }
            }
        ...
        public static async Task<byte[]> GetImageAsByteArray(string imageUrl)
        {
        ...
        public async Task<byte[]> CropAndResizeImage2(byte[] imageBytes)
        {
            using (MemoryStream memoryStream = new MemoryStream(imageBytes))
            {
                using (Android.Graphics.Bitmap originalBitmap = BitmapFactory.DecodeStream(memoryStream))
                {
                    ...
                    int[] squareSelection = await ShowSquareSelectionDialog(originalBitmap);
        ...
        internal async Task<int[]> ShowSquareSelectionDialog(Android.Graphics.Bitmap originalBitmap)
        {
            var completionSource = new TaskCompletionSource<int[]>();
           /* var selectedArea = new int[3];
